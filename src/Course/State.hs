...
firstRepeat as = eval (findM detectDuplicate as) S.empty
...
isHappy =
  contains 1 .
    firstRepeat .
    produce (sum . squareDigits)

squareDigits :: Int -> List Int
squareDigits n =
  listh (join (*) . digitToInt <$> show n)
